...
def encode_audios(audios, model, feature_extractor, device):
    ...
    for idx in tqdm(range(0, len(audio_paths), batch_size)):
        ...
        audio_read = [
            librosa.resample(
                librosa.load(i, sr=16000)[0], orig_sr=16000, target_sr=48000
            )
            for i in audio_paths[idx : idx + batch_size]
        ]

        audio_input = feature_extractor(
            audio_read, sampling_rate=48000, return_tensors="pt"
        ).to(device)
...
def evaluate(benchmark, result_path):
    ...
    audio_embedding_path = (
        f"./datasets/{benchmark}/features/audio-embeddings-query-chunking.hdf5"
    )
    ...
    ranking_path = (
        f"./datasets/{benchmark}/clap_ranking/audio-ranking-query-chunking.pkl"
    )
...
if __name__ == "__main__":
    ...
    result_path = (
        f"results/retrieved_results/{benchmark}/retrieved_audios_query_chunking.csv"
    )
