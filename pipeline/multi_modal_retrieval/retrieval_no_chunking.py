...
def compute_metrics(filtered_captions, audio_embeddings, text_embeddings):
    ...
    ranking = torch.argsort(
        torch.tensor(text_embeddings) @ torch.tensor(audio_embeddings).t(),
        descending=True,
    )
...
def evaluate(benchmark, result_path):
    ...
    audio_embedding_path = (
        f"./datasets/{benchmark}/features/audio-embeddings-no-chunking.hdf5"
    )
...
if __name__ == "__main__":
    ...
    result_path = (
        f"results/retrieved_results/{benchmark}/retrieved_audios_no_chunking.csv"
    )
