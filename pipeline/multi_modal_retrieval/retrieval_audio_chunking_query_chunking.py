    ...
    for idx in tqdm(range(len(split_captions))):
        ...
        for ii in range(1, num_of_splits - 1):
            ...
            with torch.no_grad():
                ...
                input_ids = tokenizer(caption, padding=True, return_tensors="pt").to(
                    device
                )

                text_embeddings.append(
                    model.get_text_features(**input_ids).cpu().numpy()
                )
...
def compute_metrics(split_captions, audio_chunks_embeddings, split_text_embeddings):
    ...
    for idx in tqdm(range(len(split_text_embeddings))):
        ...
        for split_embedding in query_embeddings:
            ...
            audios_score = compute_score_audio_chunk(
                split_embedding, audio_chunks_embeddings
            )
...
def evaluate(benchmark, result_path):
    ...
    split_text_embeddings = encode_captions(
        split_captions, clap_model, tokenizer, device
    )
    ...
    ranking = compute_metrics(
        split_captions, audio_chunks_embeddings, split_text_embeddings
    )
