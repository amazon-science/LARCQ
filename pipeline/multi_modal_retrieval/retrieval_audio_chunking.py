        ...
        captions.append(
            {
                "audio_id": audio_id,
                "caption": caption,
                "file_name": audios_path + audio_id,
            }
        )
...
def filter_captions(data):
    ...
    for idx in range(0, len(data), bs):
        encodings += tokenizer.batch_encode_plus(
            captions[idx : idx + bs], return_tensors="np"
        )["input_ids"].tolist()
    ...
    for audio_id, caption, encoding in zip(audio_ids, captions, encodings):
        ...
        if len(encoding) <= 512:
            ...
            print(
                "Ruled out auido files whose caption embeddings are longer than 512:",
                audio_id,
            )
...
def encode_captions(captions, model, tokenizer, device):
    ...
    for idx in tqdm(range(0, len(captions), bs)):
        ...
        with torch.no_grad():
            input_ids = tokenizer(
                captions[idx : idx + bs], padding=True, return_tensors="pt"
            ).to(device)
...
def encode_10_second_audio_chunks(
    audios, chunk_duration, model, feature_extractor, device
):
    ...
    for idx in tqdm(range(len(audio_paths))):
        ...
        for audio_chunk in audio_chunks:
            audio_input = feature_extractor(
                audio_chunk, sampling_rate=48000, return_tensors="pt"
            ).to(device)

            with torch.no_grad():
                chunk_embeddings.append(
                    model.get_audio_features(**audio_input).detach().cpu().numpy()
                )
...
def compute_metrics_audio_chunks(
    filtered_captions, audio_chunks_embeddings, text_embeddings
):
    ...
    print(
        f"Text-to-audio Retrieval Results: "
...
def evaluate(benchmark):
    ...
    audio_embedding_path = (
        f"./datasets/{benchmark}/features/audio-embeddings-audio-chunking.pkl"
    )
    ...
    if os.path.isfile(audio_embedding_path):
        ...
        audio_chunks_embeddings = encode_10_second_audio_chunks(
            audios, chunk_duration, clap_model, feature_extractor, device
        )
    ...
    ranking = compute_metrics_audio_chunks(
        filtered_captions, audio_chunks_embeddings, text_embeddings
    )

    # save rank list to a pickle file
    ranking_path = (
        f"./datasets/{benchmark}/clap_ranking/audio-ranking-audio-chunking.pkl"
    )
...
if __name__ == "__main__":
    ...
    clap_model = ClapModel.from_pretrained(
        "./models/" + clap_model_name, local_files_only=True
    ).to(device)
    ...
    result_path = (
        f"results/retrieved_results/{benchmark}/retrieved_audios_audio_chunking.csv"
    )
