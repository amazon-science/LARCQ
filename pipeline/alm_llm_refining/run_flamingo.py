    ...
    for ii in tqdm(range(len(items))):
        ...
        if is_dialogue:
            ...
            if do_dialogue_last:
                ...
                outputs = inference(
                    model, text_tokenizer, staged_item, processed_item, inference_kwargs
                )[0]
            ...
            outputs = inference(
                model, text_tokenizer, item, processed_item, inference_kwargs
            )
...
def load_data(benchmark):
    ...
    for _, item in info.iterrows():
        ...
        query = item["query"]
        queries.append(
            {"audio_id": audio_id, "query": query, "file_name": audios_path + audio_id}
        )
...
if __name__ == "__main__":
    ...
    for idx in range(len(queries)):
        ...
        item = {
            "name": audio_path,
            "prefix": "The task is audio reasoning.",
    ...
    csv_path = (
        f"results/retrieved_results/{benchmark}/retrieved_audios_{retrieval_type}.csv"
    )
